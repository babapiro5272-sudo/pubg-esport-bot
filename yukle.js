...
// 1. .env dosyasından Token ve Client ID'yi otomatik oku
let token = '';
let clientId = '';
...
// Eğer .env okunamadıysa manuel kontrol
if (!token || !clientId) {
  console.log('❌ .env dosyasında TOKEN veya CLIENT_ID bulunamadı.');
  process.exit(1);
}

// 2. Tüm Komutların Tam JSON Şeması
const commands = [
  {
    name: 'senkronize',
...
// 3. Doğrudan Discord API'sine Gönder
async function register() {
  console.log('🚀 Discord API ile bağlantı kuruluyor...');

  // Botun bulunduğu sunucuları bulmak için bot profilini çek
  const guildRes = await fetch('https://discord.com/api/v10/users/@me/guilds', {
    headers: { Authorization: `Bot ${token}` }
  });

  if (!guildRes.ok) {
    console.error('❌ Token geçersiz veya API hatası:', await guildRes.text());
    return;
  }
  ...
  for (const g of guilds) {
    console.log(`⚡ "${g.name}" (${g.id}) sunucusuna komutlar doğrudan işleniyor...`);
    const res = await fetch(`https://discord.com/api/v10/applications/${clientId}/guilds/${g.id}/commands`, {
      method: 'PUT',
      headers: {
    ...
    if (res.ok) {
      console.log(`✅ "${g.name}" sunucusuna tüm yeni komutlar yüklendi!`);
    } else {
      console.log(`⚠️ Sunucuya yüklenirken hata:`, await res.text());
    }
  }
  ...
  console.log('\n🎉 TÜM KOMUTLAR DISCORD\'A BAŞARIYLA KAYDEDİLDİ!');
}
